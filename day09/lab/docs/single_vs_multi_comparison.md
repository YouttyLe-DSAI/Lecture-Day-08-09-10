...
> Số liệu Day 09 từ trace thật. Day 08 dùng N/A vì nhóm không có artifact từ Day 08.
...
|--------|----------------------|---------------------|-------|---------|
| Avg confidence | N/A | 0.605 | N/A | Estimated từ chunk similarity scores |
| Avg latency (ms) | N/A | 7,390 | N/A | Bao gồm embedding + retrieval + synthesis |
| Abstain rate (%) | N/A | 6.7% | N/A | 1/15 câu (q09 ERR-403-AUTH) confidence < 0.45 |
| Multi-hop accuracy | N/A | 2/2 (100%) | N/A | q13 + q15 đều route đúng multi-hop |
| Routing visibility | ✗ Không có | ✓ Có route_reason | N/A | Mỗi trace có route + reason rõ ràng |
| Debug time (estimate) | ~15 phút | ~3 phút | -12 phút | Nhờ trace log + worker isolation |
| MCP usage rate | N/A | 20% | N/A | 4/20 traces có MCP tool calls |
...
|---------|--------|--------|
| Accuracy | N/A | Cao — retrieve đúng source cho q01, q02, q04, q05 |
| Latency | N/A | ~5,500ms (1 retrieval + 1 synthesis) |
| Observation | Một prompt duy nhất, không trace | 2 workers, trace rõ ràng per-step |

**Kết luận:** Multi-agent không cải thiện accuracy cho câu đơn giản (đã đủ tốt ở single-agent) nhưng trace rõ hơn. Latency tăng nhẹ vì overhead của supervisor routing + worker IO logging.

### 2.2 Câu hỏi multi-hop (cross-document)

| Nhận xét | Day 08 | Day 09 |
|---------|--------|--------|
| Accuracy | N/A | 2/2 multi-hop đúng (q13, q15) |
...
Multi-agent tốn thêm ~5s latency so với single agent (do embedding model load + ChromaDB query + worker chaining). Tuy nhiên, benefit lớn nhất là trace visibility: khi answer sai, debug time giảm từ 15' xuống 3'. Trong production, MCP tools có thể gọi qua HTTP → latency tăng nhưng extensibility tốt hơn.
...
1. **Câu hỏi đơn giản** — latency tăng (5.5s vs ~2s single-agent) mà accuracy không cải thiện. Overhead không đáng với simple factual questions.
...
3. Real MCP HTTP server (bonus +2) để decouple tools deployment
