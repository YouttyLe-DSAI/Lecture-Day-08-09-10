...
Công việc của tôi đóng vai trò là "chốt chặn" cuối cùng của dự án. Sau khi các bạn phụ trách Supervisor (Sprint 1) và Workers (Sprint 2, 3) hoàn thành logic chạy, tôi sử dụng file `eval_trace.py` để "stress test" toàn bộ 15 câu hỏi của hệ thống. Tôi thu thập dữ liệu về độ trễ (latency), độ tự tin (confidence) và log routing từ Supervisor để tạo ra báo cáo so sánh khách quan với hệ thống Single-agent của Day 08. Nếu không có phần việc của tôi, nhóm sẽ không thể chứng minh được sự ưu việt của kiến trúc Multi-Agent về mặt định lượng.
...
**Quyết định:** Triển khai cơ chế lưu trữ "Double-Tracking Trace" (Lưu trữ dấu vết kép).
...
Các lựa chọn thay thế là chỉ dùng một file JSONL duy nhất (như phần grading), nhưng JSONL lại khó đọc bằng mắt thường (Human-readable). Vì vậy, tôi chọn giải pháp duy trì file lẻ để phục vụ audit chi tiết (khi cần soi kỹ 1 câu cụ thể) và file gộp để phục vụ phân tích metrics tổng hợp.
