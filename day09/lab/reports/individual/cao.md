...
`graph.py` import trực tiếp `workers.retrieval.run`, `workers.policy_tool.run`, `workers.synthesis.run` — interface của Ly. MCP integration của Nam được gọi gián tiếp qua policy_tool worker. Tuấn dùng `run_graph()` và `save_trace()` trong eval_trace.py.

**Bằng chứng:** Xem comment `# Owner: Cao` trong graph.py, dòng 1 và section 2, 6.
...
Workers run() functions từ Ly — tôi cần interface match contracts. MCP dispatch_tool() từ Nam.

---

## 5. Nếu có thêm 2 giờ, tôi sẽ làm gì?

Tôi sẽ thêm **LLM-based routing fallback** cho câu mà keyword matching miss (confidence < 0.3 sau retrieval). Lý do: trace q09 cho thấy keyword matching route default → retrieval → chunks không liên quan → confidence 0.41. LLM classifier có thể detect "unknown error code" → human_review, tránh hallucination.

---

*Lưu file này với tên: `reports/individual/cao.md`*
